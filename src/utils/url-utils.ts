...
export function getPostUrlBySlug(slug: string, published?: Date | string): string {
	// 移除文件扩展名（如 .md, .mdx 等）
	...
	if (published) {
		...
		if (!Number.isNaN(date.valueOf())) {
			...
			return url(`/${year}/${month}/${day}/${slugWithoutExt}/`);
		}
	}
...
export function isPostPath(pathname: string): boolean {
	...
	return path.startsWith("/posts/") || /^\/\d{4}\/\d{2}\/\d{2}\/.+/.test(path);
}
